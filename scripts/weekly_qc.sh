...
# mailx -s "[$SERVER] Checking for duplicate records finished." $DEVELOPER_EMAIL < $OUTPUT_FILE
# WC_RESULT=`wc -l $OUTPUT_FILE | awk '{print $1}'`
# if [[ $WC_RESULT != '1' ]]
# then
  # echo "Emailing result to curator."
  # mailx -s "[$SERVER] Checking for duplicate records finished." $CURATOR_EMAIL < $OUTPUT_FILE
  ...
  # mailx -s "[$SERVER] Updating status of duplicate records finished." $DEVELOPER_EMAIL < /dev/null
# fi
